...
data = readtable('input/WaveformScan_ASIC_-40C.dat');
time = data.tc(1:120);
ch_out = nan(size(time, 1), 32);
counter = 1;
...
for pt = 0:7
    ...
    ax.Legend.FontSize = fontsize; 
    f.Position = [200 160 900  550];
    exportgraphics(gcf,['output/WAVEFORM_ASIC_-40C_pt', num2str(pt) ,'.pdf'],'ContentType','vector');
end
...
%ax.Legend.FontSize = fontsize; 
%f.Position = [200 160 900  550];
exportgraphics(gcf,'output/WAVEFORM_ASIC_-40C_allpt.pdf','ContentType','vector');
print(f, 'output/WAVEFORM_ASIC_-40C_allpt','-dsvg')
